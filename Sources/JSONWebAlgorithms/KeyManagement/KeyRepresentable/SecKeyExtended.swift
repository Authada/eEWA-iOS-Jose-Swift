...
public struct SecKeyExtended {
    ...
    public let isSecureEnclaveKey: Bool
    ...
    public init(secKey: SecKey) throws {
        ...
        // Extract key data
        if let keyClass = attributes[kSecAttrTokenID as String] as? String, keyClass == kSecAttrTokenIDSecureEnclave as String {
            //SecureEnclave Key
            self.isSecureEnclaveKey = true

            guard let toid = attributes["toid"] as? Data else {
                throw SecKeyError.keyDataExtractionFailed
            }

            self.keyData = toid
        }
        else {
            self.isSecureEnclaveKey = false
            // Extract key data
    ...
    private func buildP256Key() throws -> JWK {
        if isSecureEnclaveKey && isPrivate {
            switch keyUsage {
            case .sign, .unknown:
                let key = try SecureEnclave.P256.Signing.PrivateKey.init(dataRepresentation: keyData)
                return key.jwkRepresentation
            case .keyAgreement:
                let key = try SecureEnclave.P256.KeyAgreement.PrivateKey.init(dataRepresentation: keyData)
                return key.jwkRepresentation
            }
        }
        else if isPrivate {
